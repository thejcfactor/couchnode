...
export class ConnSpec {
  ...
  static parse(connStr: string, api = ApiImplementation.Classic): ConnSpec {
    const spec = new ConnSpec(undefined, api)
  ...
  toString(skipScheme?: boolean): string {
    let connStr = ''

    if (this.scheme && !(skipScheme ?? false)) {
      connStr += this.scheme + '://'
    }
